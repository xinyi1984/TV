...
需要 Node.js 22.18+（22.x）或 Node.js 24+（測試與預覽使用 Node 內建的 TypeScript 型別移除功能）。
...
`npm test` 會先建置（包含 TypeScript 型別檢查），再檢查內容、JSON、路徑及互動邏輯。`npm start` 只提供 `out/` 內的檔案，網址為 `http://localhost:3000/`；它不是正式部署所需的伺服器。請透過 HTTP 預覽，不要直接雙擊 HTML。
...
3. 流程在 `website/` 安裝依賴、執行 Lint、Android 來源契約核對、型別檢查與測試，再發布 `website/out/`。
...
預覽網址此時為 `http://localhost:3000/TV/`。`SITE_URL` 是建置時設定，測試及預覽應使用與建置相同的值；更換網域或路徑需重新建置。完成後以 `Remove-Item Env:SITE_URL` 清除設定。

## Android 契約核對

網站建置不依賴 Android 工具鏈。在 `website/` 可另行執行來源契約核對；發布流程也會執行：

```sh
npm run test:source
```

這會比對 `Result.java`、本地 API 動作與回傳欄位，並以 Python 執行文件 Demo 與 TV 實際橋接函式；需要相鄰 TV 原始碼與 Python 3。缺少來源會明確失敗，不會默默略過。修改介接規格或範例時，應一併執行。
